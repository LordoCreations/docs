...
<Tabs>
  ...
  <Tab title="Android Studio">
    ...
    7. To boot programs onto the robot, ensure the robot is either connected to the computer via USB cable or wifi using [ADB WiFi](#adb_wifi), ensure "TeamCode" is selected on the top bar and press the "Run" or "Restart" button

       ![](/images/run_androids.png)

       <Info>A disadvantage of using Android Studio to boot code onto the robot is that it can take a long time --- expect times ranging from 30 seconds to 2 minutes</Info>
  </Tab>

  <Tab title="ADB WiFi">
    ## <span id="adb_wifi">ADB WiFi</span>
  ...
  <Tab title="Roadrunner">
    ...
    Roadrunner is a robot localization, path following, and action library designed for Autonomous period robot control.
  </Tab>
</Tabs>